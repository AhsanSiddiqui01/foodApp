...
const Api = {
   loginPost : async  (endPoint, credentials, success, error, formdata = false) =>{
    ...
    axios({
      "method": "POST",
      "url": `${config.ReportUrl}/login.php`,
      "headers": {
      ...
      .then(async(response) => {
        ...
        console.log('USER RESULT:', setItm.session_id)
        if(setItm.data === true){
...
get: (endPoint, success, error) => {
  ...
  let ReportUrl = config.ReportUrl + endPoint;
  // console.log('getttttttt',ReportUrl)
  let headers = {
    "Content-Type" : "application/octet-stream",
    "x-rapidapi-host":"quotes15.p.rapidapi.com",
  ...
  // console.log('ReportUrl  ', ReportUrl, '  ', _state);
  console.log('UserNameAccess  ', _state.GeneralReducer.name);
  let requestConfig = {
    method: 'GET',
